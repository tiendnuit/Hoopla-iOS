//
//  CGFloat+Layout.swift
//  Hoopla
//
//  Created by Scor Doan on 04/02/2021.
//

import Foundation
import UIKit

// MARK: - Layout

extension CGFloat {

    /// Default spacing in the app
    public static var defaultSpacing: CGFloat { 16 }
    /// Default cell height
    public static var defaultCellHeight: CGFloat { 270 }
    /// Default number of items per row
    public static var defaultItemsPerRow: CGFloat { 2 }
    /// Separated line height
    public static var bottomLineHeight: CGFloat { 1 }
    /// Components spacing
    public static var defaultComponentsSpacing: CGFloat { 8 }
    /// Corner radius of thumbnail images
    public static var thumbnailRadius: CGFloat { 8 }
    /// Default Corner radius
    public static var defaultRadius: CGFloat { 4 }
    /// textfield height
    public static var textFieldHeight: CGFloat { 40 }
    /// Height of the `primary` thumbnail image
    public static var thumbnailHeight: CGFloat { 240 }
    /// Height of the button
    public static var buttonHeight: CGFloat { 50 }
}

